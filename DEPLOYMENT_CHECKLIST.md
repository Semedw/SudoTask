...
| 7 | Static files pipeline is incomplete | `STATIC_ROOT` exists, but no explicit production `collectstatic` + serving path is wired | Add `collectstatic` step and static file serving via proxy (or Whitenoise strategy) | done |
